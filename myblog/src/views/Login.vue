...
<div class="login">
	<div>
		<router-link to="/Home"><button class="frbtn">进入主页</button></router-link>
		...
		<div class="bg pre">
			...
			<div v-show="show">
				...
				<h4>Sign up</h4>
				<div class="tal">
					<input type="text" name="zc" v-model="newUsername" placeholder="请输入用户名注册"><br>
		...
		login(){
			if (this.username === "" || this.password === ""){
				...
				if(flaglogin){
					...
					this.$route.params.name = this.username;
					LocalStorage.saveName(this.$route.params.name);
					// console.log(this.$route.params.name);
					if(redirect){
						this.$router.push(redirect);
					}else{
						this.$router.replace("/home/picture");
					}
				}else{
...
<style scoped>
	...
	.frbtn{
		...
		height: 45px;
		border-radius: 5px;
		background-color: rgba(131,175,155,.6);
